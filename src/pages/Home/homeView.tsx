...
const HomeView: React.FC<Props> = ({ loading, data }) => {
  ...
  const columns: GridColDef[] = [
    ...
    { field: 'salario_base', headerName: 'Salario base', width: 120 },
    { field: 'salario_total', headerName: 'Salario total', width: 120, flex: 1, renderCell: (row: any) => row.row.salario_total ? row.row.salario_total : 'No asignado' },
    { field: 'eficiencia', 
      headerName: 'Eficiencia', 
      width: 120,
      flex: 1, 
      renderCell: (row: any) => row.row.confecciones_totales ? Number(row.row.confecciones_totales) * 100 / Number(row.row.confecciones_minimas) : 'No asignado' },
    {
      field: ' ',
      headerName: ' ',
      sortable: false,
